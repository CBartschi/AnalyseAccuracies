...
```
## RKHS matrix effect
```{r rkhs matrix effect, echo=FALSE}
datarkhs <- dataPA[dataPA$CV %in% c("IMBcdm_Gmat", "IMBcdm_Kmat", "IMBran") & dataPA$method=="RKHS",]
...
datarkhs$s <- as.factor(datarkhs$s)
  ...
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(0.5))+
  geom_point(aes(color = CV, shape = Predictor), position=position_dodge(0.5))+ #shape = Proxy
  facet_grid(trait~method, scales = "free_y")+
...
rkhs_lmMain <- dlply(datarkhs, .(trait), function(x) lm_out <- list(lmobj=lm(z ~ Predictor + CV + s, data =x[x$CV!="IMBran",]),
                                                                  tr=x$trait[1]))

summTable_rkhs <- ldply(.data=rkhs_lmMain, .fun=summTable_fun)

kable(summTable_rkhs)
...
plot <- 
  ...
  # theme(strip.background =element_blank(),
  #       strip.text = element_text(colour = "black"))
  ggplot(dataPA[dataPA$CV != "CV2" & dataPA$Predictor %in% c("LVS02", "LVS03"),], 
       aes(x=xlabel, y= mean, group=CV, label=sprintf("%0.2f", round(mean, digits = 2))))+
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(0.5))+
  geom_point(aes(color = Predictor, shape = CV), position=position_dodge(0.5))+ #shape = Proxy
  facet_grid(trait~method)+
