...
knitr::opts_knit$set(root.dir=paste0(root,"OneDrive/A_GenomicPrediction/PredictionASReml/GS_VL/S02S03_rerun/AnalyseAccuracies"))
# output
...
Acc <- droplevels(Acc[Acc$CV  %in% c("CV2", "IMBran"),])
...
dataPA <- summAcc[summAcc$CV  %in% c("CV2", "IMBran"),]
Acc <- Acc[Acc$CV!="IMBcdm",]
```
...
fileN <- paste0("output_noCDm/",
                         "Fig_trait_summAcc_noCDm.png")
...
plot1 <- 
  ...
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(0.5))+
  geom_point(aes(color = Generation), position=position_dodge(0.5))+ #shape = Proxy
  facet_grid(trait~method)+
  # ggtitle("Size comparison")+
  # ylab("Cor(GEBV, Yref)")+
  ylab("cor(GEBV, Yref)")+
  ...
  theme(strip.background =element_blank(),
        strip.text = element_text(colour = "black"))
fileN <- paste0("output_noCDm/",
                         "Fig_sizeEffect_summAcc_long_noCDm.png")
ggsave(filename = fileN,
...
plot2 <- 
  ...
  theme(strip.background =element_blank(),
        strip.text = element_text(colour = "black"))
fileN <- paste0("output_noCDm/",
                         "Fig_sizeEffect_allAcc_long_noCDm.png")
ggsave(filename = fileN,
...
plot1 <- 
  ...
  ggplot(dataPA[dataPA$LOC == "SRO" & dataPA$s %in% c("200", "234"),], aes(x=Generation,y=mean, group=CV))+
    ...
    theme(strip.background =element_blank(),
          strip.text = element_text(colour = "black"))
fileN <- paste0("output_noCDm/",
                         "Fig_samplingEffect_summAcc_long_noCDm.png")
ggsave(filename = fileN,
       plot1, width = 16, height = 20, units = "cm")
cat(fileN)
...
fileN <- paste0("output_noCDm/",
                         "Fig_samplingEffect_allAcc_long_noCDm.png")
ggsave(filename = fileN,
       plot2, width = 16, height = 20, units = "cm")
cat(fileN)
...
dataPA <- droplevels(Acc[Acc$CV %in% c("CV2", "IMBran"),])
```
...
size_lmMain$FL$lmobj$coefficients
...
fileN <- paste0("Analyse_sizeInter_n",
                      n,
                      "_",
                      today,
                      ".csv")
...
## correlation YLD ZN
```{r}
corr_infile <- "Prediction_YLD-ZN_forCorr.csv"
YLD_ZN_summ <- read.csv(corr_infile,
                        header = TRUE)
...
Prediction file: `r corr_infile`

```{r correlation Ref. echo=FALSE}

ggplot(YLD_ZN_summ[YLD_ZN_summ$CVround==1,], aes(x=ref_YLD, y=ref_ZN, group=CV))+
